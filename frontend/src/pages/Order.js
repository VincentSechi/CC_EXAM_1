...
const Order = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const sanitizedShippingAddress = Object.entries(shippingAddress).reduce(
        (acc, [key, value]) => ({
          ...acc,
          [key]: (value || "").trim(),
        }),
      ...
      const message =
        error.response?.data?.message || error.message || "Une erreur technique est survenue. Veuillez réessayer.";
