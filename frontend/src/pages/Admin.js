...
const Admin = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setErrorMessage("Impossible de charger les données d'administration.");
      }
    };
  ...
  const handleStockUpdate = async (productId) => {
    ...
    try {
      ...
      alert(`Stock du produit mis à jour à ${parsedStock}.`);

      const updatedProducts = products.map((product) =>
        product._id === productId ? { ...product, stock: parsedStock } : product
      );
